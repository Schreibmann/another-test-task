...
const main = async () => {
    ...
    await Promise.all(
      ...
      Object.keys(balance).map(currency => {
        ...
        if (currency !== 'BTC') {
          ...
          let pair = (currency === 'USDT') ? 'BTCUSDT' : (currency === 'BCH') ? 'BCCBTC' : currency + 'BTC';
          ...
          return fetch(`${marketApiURI.binance}${pair}`)
            .then(res => {
              ...
              .then(binanceData => {            
                  ...
                  pair = (currency === 'BCH') ? 'btc-bcc' : (currency === 'USDT') ? 'usdt-btc' : 'BTC-' + currency; 
                  ...
                  return fetch(`${marketApiURI.bittrex}${pair.toLowerCase()}`)
                    .then(res => {
                      ...
                      .then(bittrexData => {
                        ...
                        let bittrexPrice = (currency === 'USDT') ? (1/bittrexData.result[0].Last) : bittrexData.result[0].Last;
                        let binancePrice = (currency === 'USDT') ? (1/binanceData.price) : binanceData.price;
                        
                        console.log(`|  ${currency.padStart(4, " ")}:  ${balance[currency].toFixed(8).padStart(15, " ")}    price at bittrex: ${parseFloat(bittrexPrice).toFixed(8)}    price at binance: ${parseFloat(binancePrice).toFixed(8)}`);
  ...
  const printEvalBalances = () => {
    console.log('');
    console.log(`* Balance BTC evaluation at binance is ${balanceInBTCatBinance} BTC `);
    console.log(`* Balance BTC evaluation at bittrex is ${balanceInBTCatBittrex} BTC `);
  };
